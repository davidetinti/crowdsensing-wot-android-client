...
public class RefCountResource<T> {
	...
	public RefCountResource(Supplier<? extends T> resourceSupplier, Consumer<? super T> resourceCleanup) {
		this(resourceSupplier, resourceCleanup, new AtomicInteger(0), new AtomicReference(null));
	}

	RefCountResource(Supplier<? extends T> resourceSupplier, Consumer<? super T> resourceCleanup, AtomicInteger refCount, AtomicReference<T> resource) {
	...
	public T retain() throws RefCountResourceException {
		synchronized (this) {
			if (refCount.getAndIncrement() == 0) {
				try {
					Log.d("RefCountResource", "First Reference. Create Resource.");
	...
	public void release() throws RefCountResourceException {
		synchronized (this) {
			if (refCount.updateAndGet(i -> i > 0 ? i - 1 : i) == 0 && resource.get() != null) {
				Log.d("RefCountResource", "No more References. Cleanup Resource: " + resource.get());
				try {
