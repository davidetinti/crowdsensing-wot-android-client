...
public class AndroidPahoConnector implements BrokerConnector {

	private boolean connected;
	private MqttAndroidClient client;
	private boolean eventInitialized;
	private AndroidMQTTClient sClient;

	public AndroidPahoConnector() {
	...
	public CompletableFuture<Boolean> connect(BrokerConf conf) {
		return CompletableFuture.supplyAsync(() -> {
			try {
				this.client = new MqttAndroidClient(((AndroidBrokerConf) conf).context, conf.url, conf.id);
	...
	public CompletableFuture<Boolean> subscribe(String topic, MQTTClient mClient) {
		return CompletableFuture.supplyAsync(() -> {
			this.sClient = (AndroidMQTTClient) mClient;
			try {
				if (this.connected) {
					this.client.subscribe(topic, 3);
	...
	private void recvMessage() {
		AndroidPahoConnector that = this;
